...
make_dir_table <- function(out_dir,
  ...
  recurse_levels <- length(dir_levels) - length(stringr::str_split(gsub(pattern = "^/", replacement = "", x = out_dir), pattern = "\\/")[[1]])

  folders <- fs::dir_ls(out_dir, recurse = recurse_levels, type = "directory") |>
    gsub(pattern = "^/", replacement = "") #to handle linux 'drive' naming
